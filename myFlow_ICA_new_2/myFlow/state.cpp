...
//增量计算A边
void incrementCompute_A(KeyEdgeSet *key_edge_set, Graph& g, int source, int sink, int BreakEdge, Lower_subGraph*StateMtrix)
{
	int edge_num = key_edge_set->A_EdgeInfo[BreakEdge].Edge;
	...
	for (int ii=1; ii<= StateMtrix->State_Num; ii++)
	{
		...
		if (DinicByVertor(TempV_low,g,edge_num,source,sink) == key_edge_set->A_EdgeInfo[BreakEdge].ChangeAmount_c)
		{
			vector<int> v;
		...
		else if (DinicByVertor(TempV_upper,g,edge_num,source,sink) <  key_edge_set->A_EdgeInfo[BreakEdge].ChangeAmount_c)
		{
			//do nothing
	...
	key_edge_set->A_EdgeInfo[BreakEdge].ChangeAmount_p1 = getP1ByStateSection(allStateSection, g);
	//最大流可靠性（容量可靠性）为所有满足最大流的概率之和
	key_edge_set->A_EdgeInfo[BreakEdge].ChangeAmount_p2 = getP2ByStateSection(allStateSection, g, edge_num);
...
void sortKeyEdge_A(KeyEdgeSet * key_edge_set, Graph& g, int source, int sink, Lower_subGraph *StateMtrix)
{
	...
	while(j <= key_edge_set->A_num)
	{
		...
		}else
		{
			...
			incrementCompute_A(key_edge_set, g, source, sink, i, StateMtrix);
			...
			for (int k =j; k <= key_edge_set->A_num; k++)
			{
				...
				//i和j一样，计算j的变化p1和p2
				incrementCompute_A(key_edge_set, g, source, sink, k, StateMtrix);
...
//重新计算一个A边
void reCompute_B1(KeyEdgeSet *key_edge_set, Graph& g, int source, int sink, int BreakEdge)
{
	int edgeNum = key_edge_set->B1_EdgeInfo[BreakEdge].Edge;
	...
	key_edge_set->B1_EdgeInfo[BreakEdge].ChangeAmount_p1 = new_p1;
	key_edge_set->B1_EdgeInfo[BreakEdge].ChangeAmount_p2 = new_p2;
...
void sortKeyEdge_B1(KeyEdgeSet * key_edge_set, Graph& g, int source, int sink)
{
	...
	{
		reCompute_B1(key_edge_set, g, source, sink, i);
	}


	//排序，先根据冒泡排序p1
	for (int ia =1; ia< key_edge_set->B1_num; ia++)
	{
		for (int ja = ia+1; ja<= key_edge_set->B1_num; ja++)
		{
			if (key_edge_set->B1_EdgeInfo[ja].ChangeAmount_p1 < key_edge_set->B1_EdgeInfo[ia].ChangeAmount_p1)
			{
				Exchange_KeyEdge_B1(key_edge_set, ia,ja);
			}
		}
	}
...
//重新计算一个A边
void reCompute_B2(KeyEdgeSet *key_edge_set, Graph& g, int source, int sink, int BreakEdge)
{
	int edgeNum = key_edge_set->B2_EdgeInfo[BreakEdge].Edge;
	...
	//将断掉边之后的最大流，分布可靠性，容量可靠性分别保存
	key_edge_set->B2_EdgeInfo[BreakEdge].ChangeAmount_p1 = new_p1;
	key_edge_set->B2_EdgeInfo[BreakEdge].ChangeAmount_p2 = new_p2;
...
//排序一段p2  A边
void Bubbling_KeyEdge_B2_2(KeyEdgeSet *key_edge_set, int begin_, int end_)
{
	/*需要满足i<j, 将从i到j的区间排序*/
	if (begin_ > end_)
	{
...
void sortKeyEdge_B2(KeyEdgeSet * key_edge_set, Graph& g, int source, int sink)
{
	...
	{
		reCompute_B2(key_edge_set, g, source, sink, i);
	}


	//排序，先根据冒泡排序p1
	for (int ia =1; ia< key_edge_set->B2_num; ia++)
	{
		for (int ja = ia+1; ja<= key_edge_set->B2_num; ja++)
		{
			if (key_edge_set->B2_EdgeInfo[ja].ChangeAmount_p1 < key_edge_set->B2_EdgeInfo[ia].ChangeAmount_p1)
			{
				Exchange_KeyEdge_B2(key_edge_set, ia,ja);
			}
		}
	}
	...
	int iii = 1;
	int jjj=iii+1;
	while(jjj<=key_edge_set->B2_num)
	{
		if (key_edge_set->B2_EdgeInfo[jjj].ChangeAmount_p1 != key_edge_set->B2_EdgeInfo[iii].ChangeAmount_p1)
		{
			iii++;
			jjj++;
		}else
		{
			//找到相同变化的区间
			for (int kkk =jjj; kkk <= key_edge_set->B2_num; kkk++)
			{
				if (key_edge_set->B2_EdgeInfo[kkk].ChangeAmount_p1 != key_edge_set->B2_EdgeInfo[iii].ChangeAmount_p1)
				{
					jjj = kkk;
					break;
				}
				if ( kkk == key_edge_set->B2_num)
				{
					jjj = kkk+1;
					break;
				}
			...
			//使用冒泡排序一段
			Bubbling_KeyEdge_B2_2(key_edge_set, iii, jjj-1);
			iii=jjj;
			jjj=iii+1;
		}
	}
...
void computeICA(Lower_subGraph *StateMtrix, KeyEdgeSet *key_edge_set, Graph& g, int source, int sink)
{
	...
	//3.将边   定量计算    并   排序
	//A类边重新计算
	sortKeyEdge_A(key_edge_set, g, source, sink, StateMtrix);
	//B类边,重新计算
	sortKeyEdge_B1(key_edge_set, g, source, sink);
	//for class edge b 
	sortKeyEdge_B2(key_edge_set, g, source, sink);
	//C类边不需要计算
