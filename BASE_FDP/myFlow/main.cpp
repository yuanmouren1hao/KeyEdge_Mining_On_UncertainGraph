...
int _tmain(int argc, char* argv[])
{
    ...
    */
	...
	while(inReader.ReadGraph(g))                               /*读取文件中的图*/
	{
		...
		dP = GetMPMF(g,s,t,maxflow,maxPmaxF);     /*运行核心算法*/
		//cout<< dP<< endl;
		QueryPerformanceCounter((LARGE_INTEGER*)&start);      /*记录开始时间*/
		computeBase(&key_edge_set, g, s, t, out_test);        //BASE_FDP核心计算，key_edge_set用于保存计算的结果
		...
		out_test<<(double)memsize/MB<<endl;
