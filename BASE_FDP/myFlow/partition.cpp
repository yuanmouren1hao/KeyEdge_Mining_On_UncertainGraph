...
void saveAllState(StateSet c,Lower_subGraph * StateMtrix)
{
	...
	{
		StateMtrix->State[StateMtrix->State_Num][ii]=c.lower[ii];
		StateMtrix->State_upper[StateMtrix->State_Num][ii]=c.upper[ii];
	}
	return;
}
...
/*通过对可能事件模型进行划分得到最可靠最大流分布*/
double  GetMPMF(Graph& g,int source,int sink,int &maxflow,Flow& resultFd)
{	
	assert((source > 0 && source <= g.nV) && (sink > 0 && sink <= g.nV));         /*保证输入合法*/
...
/*通过对可能事件模型进行划分得到最可靠最大流分布*/
double base_GetMPMF(Graph& g, int source, int sink, int &maxflow, Flow& resultFd, int break_edge)
{	
	assert((source > 0 && source <= g.nV) && (sink > 0 && sink <= g.nV));         /*保证输入合法*/
