...
void computeP1P2(KeyEdgeSet *key_edge_set, Graph& g, int source, int sink)
{
...
void computeBase(KeyEdgeSet *key_edge_set, Graph& g, int source, int sink)
{
	...
	//2.根据最大流计算判断是否计算    分布可靠性    和     容量可靠性
	computeP1P2(key_edge_set, g, source, sink);


	//3.排序，根据流量，分布可靠性，容量可靠性  三个维度
	//排序在上一个函数已经完成
