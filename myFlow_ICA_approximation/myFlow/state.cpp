...
int StateDivision(Graph& g, vector<int> down_, vector<int> top_, int source, int sink, set<vector<int>>& newState)
{	
	int nE = g.nE;                                                                /*向量大小是不变的*/
	double dR = 0.0;                                                              /*最可靠最大流分布概率*/
	int temp_num_subgraph = 0;
...
/*B类边增量计算，变量都使用引用的方式*/
void sortKeyEdge_B(KeyEdgeSet * key_edge_set, Graph& g, int source, int sink, Lower_subGraph *StateMtrix, int allStateSection_martix[NUM_OF_SUB_GRAPH][MAX_E_NUM], int nun_sub[MAX_E_NUM][3])
{
	cout<<"<-----B EDGE--->"<<endl;
	/*如果没有B类边的个数为0*/
	if (key_edge_set->B1_num == 0 && key_edge_set->B2_num == 0)
	{
		return;
	}

	//定义一个数组保存B和b需要的数据,为了统计数据，分别为：直接过滤0X，下界过滤，上届过滤，二次划分  的区间个数
	int temp_num[5]={0};
	...
	for (int i =1; i<= key_edge_set->B1_num; i++)
	{
		...
		/*获取所有的满足最大流的状态的子图区间，注意是子图区间，2表示x，即0或者1,对于每一条边的情形都是不一样的*/
		set<vector<int>> allStateSection;

		/*定义四个变量，分别计算表示(区间可数)  一定满足最大流，  下界满足最大流，上界不满足，需要划分*/
		int num_1 =0,num_2=0,num_3=0,num_4=0;
		int subgraph_num_1=0,subgraph_num_2=0,subgraph_num_3=0,subgraph_num_4=0;
		...
		for (int ii=1; ii<= StateMtrix->State_Num; ii++)
		{
			...
			if (StateMtrix->State[ii][edge_num]==0)
			{
				...
				for (int j=1; j<=StateMtrix->Edge_Num; j++)
				{
					...
					{
						v.push_back(allStateSection_martix[ii][j]);
					}
				}
				allStateSection.insert(v);
				num_1++;
				subgraph_num_1 += allStateSection_martix[ii][0];
			}
			//该区间部分满足最大流，需要进一步的判断
			else if (StateMtrix->State[ii][edge_num]==1)
			{
				...
				if (DinicByVertor(TempV_low,g,edge_num,source,sink) == g.max_flow)
				{
					...
					allStateSection.insert(v);
					num_2++;
					subgraph_num_2 += allStateSection_martix[ii][0];
				...
				else if (DinicByVertor(TempV_upper,g,edge_num,source,sink) < g.max_flow)
				{
					...
					num_3++;
					subgraph_num_3 += allStateSection_martix[ii][0];
				}
				//下界不满足最大流，上界满足最大流的话，需要重新分解子图区间，但是分解的子图区间比较小而已
				else
				{
					//返回的是不确定关系中，依然满足最大流的子图的个数
					int a =StateDivision(g,TempV_low,TempV_upper,source,sink,allStateSection);
					num_4++;
					subgraph_num_4 += a;
				}
			}
		...
		cout<<"区间个数："<<edge_num<<"->"<<num_1<<"	"<<num_2<<"	"<<num_3<<"	"<<num_4<<endl;
		temp_num[1] += num_1;
		temp_num[2] += num_2;
		temp_num[3] += num_3;
		temp_num[4] += num_4;
		cout<<"子图个数："<<edge_num<<"->"<<subgraph_num_1<<"	"<<subgraph_num_2<<"	"<<subgraph_num_3<<"	"<<subgraph_num_4<<endl;
		cout<<"子图个数代替可靠性："<<subgraph_num_1+subgraph_num_2+subgraph_num_4<<endl<<endl;
		nun_sub[edge_num][2] =  subgraph_num_1+subgraph_num_2+subgraph_num_4;
	}
	cout<<endl;
	...
	for (int i =1; i<= key_edge_set->B2_num; i++)
	{
		...
		/*获取所有的满足最大流的状态，对于每一条边的情形都是不一样的*/
		set<vector<int>> allStateSection ;

		/*定义四个变量，分别计算表示  一定满足最大流，  下界满足最大流，上界不满足，需要划分*/
		int num_1 =0,num_2=0,num_3=0,num_4=0;
		//分别表示各个区间的子图个数
		int subgraph_num_1=0,subgraph_num_2=0,subgraph_num_3=0,subgraph_num_4=0;
	...
	//输出统计数据
	cout<<"所有区间个数："<<temp_num[1]+temp_num[2]+temp_num[3]+temp_num[4]<<endl;
	cout<<"直接过滤区间个数："<<temp_num[1]<<endl;
	cout<<"下界过滤的区间个数："<<temp_num[2]<<endl;
	cout<<"上界过滤的区间个数："<<temp_num[3]<<endl;
	cout<<"需要二次划分的区间个数："<<temp_num[4]<<endl;
	cout<<"二次划分百分比是："<<temp_num[4]*100.0/(temp_num[1]+temp_num[2]+temp_num[3]+temp_num[4])<<"%"<<endl<<endl;
	cout<<"~~~~~~~~~~~~~~~"<<endl;
...
/************************************************************************/
/*B类边增量计算，变量都使用引用的方式*/
void sortKeyEdge_b(KeyEdgeSet * key_edge_set, Graph& g, int source, int sink, Lower_subGraph *StateMtrix, int* temp_num, int allStateSection_martix[NUM_OF_SUB_GRAPH][MAX_E_NUM])
{
	/*如果没有B2类边的个数为0*/
	if (key_edge_set->B2_num == 0)
	{
	...
	/*获取所有的满足最大流的状态*/
	set<vector<int>> allStateSection ;
	//int num_1_all =0,num_2_all=0,num_3_all=0,num_4_all=0;
	...
	for (int i =1; i<= key_edge_set->B2_num; i++)
	{
		...
		temp_num[4] += num_4;
		cout<<"子图个数："<<edge_num<<"->"<<subgraph_num_1<<"	"<<subgraph_num_2<<"	"<<subgraph_num_3<<"	"<<subgraph_num_4<<endl;
		cout<<"子图个数代替可靠性："<<subgraph_num_1+subgraph_num_2 + subgraph_num_4<<endl;
	}
	cout<<"++++++++++++++"<<endl;
	return;
}
...
//根据边在极小子图中的个数判断边的关键程度
void key_by_num_in_StateMtrix(Lower_subGraph* StateMtrix, int num_sub[][3], Graph& g, int source, int sink, int allStateSection[NUM_OF_SUB_GRAPH][MAX_E_NUM])
{
	int all_subgraph_num =0;//所有子图个数
	//获取简化的子图区间，并且计算子图区间的个数，保存在子图的第0个元素中
	for (int i=1;i<=StateMtrix->State_Num;i++)
	{
		//给一个区间的0标志位表示的是该区间子图个数是，初始化为1，没有个×要加2倍
		allStateSection[i][0] = 1;
		for (int j=1; j<=StateMtrix->Edge_Num; j++)
		{
			if (StateMtrix->State[i][j]==0 && StateMtrix->State_upper[i][j]==0)
			{
				allStateSection[i][j] = 0;
			}else if (StateMtrix->State[i][j]==1 && StateMtrix->State_upper[i][j]==1){
				allStateSection[i][j] = 1;
				//*1  略去不写
			}
			else if (StateMtrix->State[i][j]==0 && StateMtrix->State_upper[i][j]==1)
			{
				allStateSection[i][j] = 2;
				allStateSection[i][0] *= 2;//子图个数*2
			}
		}
		all_subgraph_num += allStateSection[i][0];//记录总的子图个数
	}
	cout<<"子图的个数总个数:"<<all_subgraph_num<<endl;
	num_sub[0][1] = all_subgraph_num;//保存所有子图个数
	//考虑在此将子图区间进行合并，直到两两不能合并为止
	...
	for (int i=1;i<=StateMtrix->Edge_Num;i++)
	{
		...
		{
			if (allStateSection[j][i] == 1)
			{
				num_sub[i][1]+=allStateSection[j][0];
			}else if (allStateSection[j][i] == 2)
			{
				num_sub[i][1]+=(allStateSection[j][0]/2);
			}
		}
	}
	...
	for (int i=1;i<=StateMtrix->Edge_Num;i++)
	{
		cout<<num_sub[i][0]<<"->"<<num_sub[i][1]<<"->"<<num_sub[i][2]<<endl;
	}
	...
	for (int i=1;i<StateMtrix->Edge_Num;i++)
	{
		for (int j=i+1; j<=StateMtrix->Edge_Num;j++)
		{
			if (num_sub[j][1]>num_sub[i][1])
			{
				temp_int = num_sub[j][1];
				num_sub[j][1] = num_sub[i][1];
				num_sub[i][1] = temp_int;

				temp_int = num_sub[j][0];
				num_sub[j][0] = num_sub[i][0];
				num_sub[i][0] = temp_int;
			}
		}
	}

	cout<<"经过  边被子图包含个数   排序："<<endl;
	for (int i=1;i<=StateMtrix->Edge_Num;i++)
	{
		cout<<num_sub[i][0]<<"->"<<num_sub[i][1]<<"->"<<num_sub[i][2]<<endl;
	}
	...
	//根据流量排序一下
 	int A_edge_num = num_sub[0][0];
	for (int i =1; i<= A_edge_num; i++)
	{
		num_sub[i][2] = getMAXflowWithBreakEdge(g, num_sub[i][0], source, sink);
	}
	for (int i =1; i<A_edge_num;i++)
	{
		for (int j =i+1; j<=A_edge_num; j++)
		{
			if (num_sub[j][2]<num_sub[i][2])
			{
				temp_int = num_sub[j][0];
				num_sub[j][0] = num_sub[i][0];
				num_sub[i][0] = temp_int;
				...
				temp_int = num_sub[j][2];
				num_sub[j][2] = num_sub[i][2];
				num_sub[i][2] = temp_int;
			}
		}
	}
	//加上流量进行排序之后
...
{
	cout<<"子图区间的个数为："<<StateMtrix->State_Num<<endl;
	/*先计算每条边断掉之后流量减少*/
	...
	cout<<"区间个数："<<StateMtrix->State_Num<<endl;

	// 近似保存边的关键程度
	int num_sub[MAX_E_NUM][3]={0};//边号，，流量,子图个数
	for (int i=1;i<=StateMtrix->Edge_Num;i++)
	{
		num_sub[i][0] = i;
		num_sub[i][1] = getMAXflowWithBreakEdge(g, i, source, sink);
	}

	num_sub[0][0]=key_edge_set->A_num;//记录A类边的个数
	int allStateSection[NUM_OF_SUB_GRAPH][MAX_E_NUM];//保存子图区间的简化写法
	//获取子图的简化表示方式allStateSection，同时记录边被包含的子图个数保存在num_sub
	//int temp[MAX_E_NUM][3]={0};
	key_by_num_in_StateMtrix(StateMtrix, &num_sub[0], g, source, sink, &allStateSection[0]);
	//A 边断掉，0个子图满足，C边断掉一般满足
	for (int i=1;i<=key_edge_set->A_num; i++)
	{
		num_sub[key_edge_set->A_EdgeInfo[i].Edge][2] = 0;
	}
	for(int i =1; i<=key_edge_set->C_num;i++)
	{
		num_sub[key_edge_set->C_EdgeInfo[i].Edge][2] = num_sub[0][1];
	}
	...
	cout<<"B类边的个数："<<key_edge_set->B1_num+key_edge_set->B2_num<<endl;
	cout<<"B1类边的个数："<<key_edge_set->B1_num<<endl;
	cout<<"B2类边的个数："<<key_edge_set->B2_num<<endl;

	/*B类边增量计算*/
	sortKeyEdge_B(key_edge_set, g, source, sink, StateMtrix, &allStateSection[0], &num_sub[0]);
	/*for class edge b */
	//sortKeyEdge_b(key_edge_set, g, source, sink, StateMtrix, temp_num, &allStateSection[0]);
	...
	{
		//边号，流量，个数
		cout<<setw(6)<<num_sub[i][0]<<setw(6)<<num_sub[i][1]<<setw(6)<<num_sub[i][2]<<endl;
	}
